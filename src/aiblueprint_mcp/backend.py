...
def _mut(func: Callable) -> Callable:
    """Like ``_op``, but snapshots the document for undo before a successful op.

    The snapshot is taken before the operation runs and committed to the undo
    stack only if the op returns ``ok`` — so failed operations don't leave a
    no-op entry. Inside a ``batch()`` the per-op snapshot is skipped (the batch
    takes a single checkpoint for the whole sequence).
    """

    @functools.wraps(func)
    async def wrapper(self, *args, **kwargs):
        try:
            snapshot = None if self._batch_depth else self._snapshot()
        except CommandError as ce:
            return CommandResult(ok=False, error=str(ce))
        try:
        ...
        if snapshot is not None and result.ok:
            self._commit_checkpoint(snapshot)
        return result
...
class _DocState:
    ...
    counter: int = 0
    name: str = "untitled"
    # Undo/redo: each entry is a full serialized-DXF snapshot of the document.
...
class AIBlueprintBackend:
    ...
    _UNDO_LIMIT = 50  # max snapshots retained per document
    ...
    def _snapshot(self) -> str:
        """Serialize the current document (raises if none open)."""
        return self._serialize(self._state().doc)

    def _commit_checkpoint(self, snapshot: str) -> None:
        st = self._state()
        st.undo_stack.append(snapshot)
        if len(st.undo_stack) > self._UNDO_LIMIT:
            st.undo_stack.pop(0)
        st.redo_stack.clear()

    def _restore(self, st: _DocState, snapshot: str) -> None:
        doc = ezdxf.read(io.StringIO(snapshot))
        st.doc = doc
        st.msp = doc.modelspace()

    @contextmanager
    def batch(self) -> Iterator[None]:
        """Group nested mutating ops into a single undo checkpoint.

        Takes one snapshot on entry and commits it on clean exit; the per-op
        snapshots inside are suppressed. Nesting is supported — only the
    ...
    async def undo(self) -> CommandResult:
        """Revert the current document to the previous checkpoint."""
        st = self._state()
        if not st.undo_stack:
            raise CommandError("Nothing to undo")
        st.redo_stack.append(self._serialize(st.doc))
        self._restore(st, st.undo_stack.pop())
        return CommandResult(ok=True, payload={
            "entity_count": len(self._msp),
            "undo_depth": len(st.undo_stack),
            "redo_depth": len(st.redo_stack),
        })
    ...
    async def redo(self) -> CommandResult:
        ...
        st.undo_stack.append(self._serialize(st.doc))
        self._restore(st, st.redo_stack.pop())
        return CommandResult(ok=True, payload={
            "entity_count": len(self._msp),
            "undo_depth": len(st.undo_stack),
            "redo_depth": len(st.redo_stack),
